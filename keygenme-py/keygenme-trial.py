...
def check_key(key, username_trial):
    ...
    if len(key) != len(key_full_template_trial):
        print("blah blash gona return false")
        return False
    else:
        # Check static base key part --v
        i = 0
        for c in key_part_static1_trial:
            # print(f"{key_part_static1_trial} - {c} - {key[i]}")
            if key[i] != c:
                return False

            i += 1
        print("after for loop")
        print(f"hashlib.sha256(username_trial).hexdigest(): {hashlib.sha256(username_trial).hexdigest()}")
        sha_in_hex = hashlib.sha256(username_trial).hexdigest()
        dynamic = sha_in_hex[4] + sha_in_hex[5] + sha_in_hex[3] + sha_in_hex[6] + sha_in_hex[2] + sha_in_hex[7] + sha_in_hex[1] + sha_in_hex[8]
        print(f"dynamic part should be: {dynamic}")
        print(f"asnwer should be: {key_part_static1_trial}{dynamic}{key_part_static2_trial}")
